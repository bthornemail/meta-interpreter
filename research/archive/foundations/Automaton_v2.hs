...
--
-- This draft implements the latest agreed role:
--
--   * accepts arbitrary byte streams
--   * does NOT execute them
--   * pattern-matches on Header8/control structure
--   * emits datasets deterministically
--   * lifts blocks/carriers into canonical points / point-sets
--
-- Prefix head law
-- ---------------
--   []                    -> [DS_Null]
...
--   [0, ...]              -> [DS_BitBoard0]
--   [x, ...], x /= NULL   -> [DS_Board0]
--   after non-NULL entry:
--      next /= FS         -> [DS_Board0]
--      next-next /= FS    -> [DS_BitBoard0, DS_Board0, DS_Artifact0,
--                              DS_ControlPoint cp, DS_PointTuple ...]
...
prefixDatasets time bytes =
  let toks = tokenizeHead8 bytes
      ...
      pt4  = pointTupleFromTokens time toks
  in case toks of
      [] ->
        [DS_Null]
