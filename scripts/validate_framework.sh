...
grep -q '"material_class":' "$tmp_dir/runtime_layers.ndjson"
grep -q '"state_class":' "$tmp_dir/runtime_layers.ndjson"
grep -q '"carrier_resolution":' "$tmp_dir/runtime_layers.ndjson"
grep -q '"artifact_class":' "$tmp_dir/runtime_layers.ndjson"
grep -q '"workflow_mode":' "$tmp_dir/runtime_layers.ndjson"
grep -q '"frame_scope_kind":' "$tmp_dir/runtime_layers.ndjson"
grep -q '"resolved_step_identity":' "$tmp_dir/runtime_layers.ndjson"
grep -q '"ui_frame_resolution":' "$tmp_dir/runtime_layers.ndjson"

first_addr="$(./bin/ttc_framework runtime < "$tmp_dir/input.bin" | sed -n '1p')"
